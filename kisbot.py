...
"""
ROS-related script for publishing data generated by spectool_raw
...
class wispy (object):
	...
	def liner(self,specline,range):
		channels=[0,0,0]
			...
			specdata=specline[specline.index(":")+2:].split()
		  	#print specdata
		  	#print "Channel 1:%d" % int(specdata[12])
       		    	#print "Channel 6:%d" % int(specdata[37])
		   	#print "Channel 11:%d" % int(specdata[62])
		   	if range == 1:
	...
	def publisher(self,queue):
		...
		while not rospy.is_shutdown():
				...
				chandata=self.liner(wisperline,1)
		                rfdata=Int16MultiArray()
				rfdata.layout.dim = [MultiArrayDimension('rfdata',3,1)]
